...
import com.nimble.lupin.pu_manager.databinding.ItemMultipleUserSelectionBinding
import com.nimble.lupin.pu_manager.models.AssignTaskModel
import com.nimble.lupin.pu_manager.views.home.schedule.assignTask.AssignTaskFragment
...
class UsersSelectionAdapter(private var itemList: List<AssignTaskModel> ) : RecyclerView.Adapter<UsersSelectionAdapter.ViewHolder>() {
    ...
    private fun addIteInSelectedList(model: AssignTaskModel){
        val userIdNotPresent =  AssignTaskFragment.selectedItemList.none { it.userId == model.userId}
        if (userIdNotPresent) {
            AssignTaskFragment.selectedItemList.add(model)
        }
    }
